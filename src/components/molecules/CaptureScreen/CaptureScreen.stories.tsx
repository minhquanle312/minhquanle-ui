...
export default {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'A component help you to capture and record on tabs or browsers.',
      },
    },
  },
...
const Template: ComponentStory<typeof CaptureScreen> = (args) => {
  ...
  return (
    <Space
      direction="vertical"
      style={{ width: '100%', justifyContent: 'center', gap: 50 }}
    >
      <Space style={{ width: '100%', justifyContent: 'center', gap: 50 }}>
        <Button onClick={() => handleShareCapture(ATTACH_KEYS.CAPTURE)()}>
          Start Capture
        </Button>
        <Button onClick={() => handleShareCapture(ATTACH_KEYS.RECORD)()}>
          Start Record
        </Button>
