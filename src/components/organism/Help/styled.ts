...
export const ButtonFeedback = styled(Button)<{ height?: number; borderRadiusCustom?: number }>`
  width: 200px;
  height: ${props => (props.height ? props.height : 36)}px !important;
  border-radius: ${props => (props.borderRadiusCustom ? props.borderRadiusCustom : 5)}px !important;
...
export const WrapperBody = styled.div<{ types?: string }>`
  padding: ${props =>
    props.types === 'help' || props.types === 'help_v1' ? 'none;' : '13px 15px;'};
  box-sizing: border-box;
...
export const WrapperIconEditor = styled.div<{ borderTop?: boolean }>`
  height: 44px;
  border: 1px solid ${THEME.token?.bw3_5};
  border-top: ${props => (props.borderTop ? `1px solid ${THEME.token?.bw3_5}` : 'none')};
  display: flex;
  align-items: center;
  padding: 0 8px;
  justify-content: space-between;
...
export const WrapperLoading = styled.div<{ width?: number | string; height?: number | string }>`
...
export const Span = styled.span<{ styles: any }>`
  font-weight: ${props => (props.styles && props.styles.length > 0 ? 'bold' : 'normal')};
`;
